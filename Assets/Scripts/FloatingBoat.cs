...
public class FloatingBoat : MonoBehaviour
{
    ...
    void LateUpdate()
    {
       ...
       ApplyFloatingRotation(); // Apply rotation
    }
    
    void ApplyFloatingRotation()
    {
        // Preserve yaw from ShipController
        float yaw = transform.rotation.eulerAngles.y;

        // Extract pitch & roll from water alignment
        Vector3 waterEuler = smoothedRotation.eulerAngles;

        Quaternion finalRotation =
            Quaternion.Euler(
                waterEuler.x,   // pitch from waves
                yaw,            // yaw from ShipController
                waterEuler.z    // roll from waves
            );
